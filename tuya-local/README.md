...
122개 제조사 표기, 313개 코드셋: 공개 라이브러리 312개와 현재 Carrier의 Tuya 104800501 코드셋입니다.
현재 Carrier는 냉방/난방 18–30°C × 풍량 4종, 자동/송풍/제습 각 1종 = 107개 켜짐 상태와 전원 끄기를 포함합니다.
**107은 기종 수가 아니라 이 리모컨 하나의 설정 조합 수입니다.**

모든 에어컨을 보장하는 제품은 아닙니다. 공개 원본 357개 중 변환 가능한 312개를 포함했습니다.
손상된 원본, 지원하지 않는 인코딩, 전송할 수 없는 상태는 제외하며 목록과 사유는 `bridge/catalog/index.json`에 있습니다.
공개 기종은 신호 변환 검사를 통과했지만 실제 에어컨에서 검증하지 않았습니다. 하나의 코드셋이 여러 모델과 호환될 수도 있습니다.
공개 코드의 반송파는 38 kHz를 사용합니다. 풍향 분기가 있는 자료는 고정된 off/auto/첫 번째 분기를 사용하며 별도 풍향 제어는 제공하지 않습니다.
기종별 일부 설정 조합은 제외될 수 있습니다. 없는 조합은 전송하지 않습니다.
...
목록에 없는 기종은 이후 코드 라이브러리 추가 또는 Tuya 내보내기가 필요합니다.
...
일상 화면은 온도 범위별 23종 프로필을 사용하며 루틴에는 표준 전원·온도·모드·풍량 capability를 유지합니다.
...
- 313개 코드셋의 74,743개 명령을 모의 Tuya 전송 계층으로 재생하고 저장된 원본 payload와 비교합니다.
